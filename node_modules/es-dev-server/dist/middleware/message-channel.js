...
const message_channel_2 = tslib_1.__importDefault(require("../browser-scripts/message-channel"));
/**
...
async function injectMessageChannelScript(ctx) {
    try {
        ...
        const reloadInjected = bodyString.replace('</body>', message_channel_2.default);
...
function createMessageChannelMiddleware(cfg) {
    return async function messageChannelMiddleware(ctx, next) {
        if (ctx.url === constants_1.messageChannelEndpoint) {
            message_channel_1.setupMessageChannel(ctx);
            return;
        }
        await next();
